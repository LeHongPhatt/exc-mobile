...
export default class ScrollPicker extends Component<any, any> {
        ...
        if (
          ...
          if (
            this.props.dataSource.some(
              (data, ____) => data === this.props.selectedData,
            ) &&
            prevProps.dataSource.length <= this.props.dataSource.length
          ) {
  ...
  renderItem = (data, index) => {
    ...
    const item = !isSelected ? (
      <TextCus body2 bgInput style={styles.dateText}>
        {data}
      </TextCus>
    ) : (
      <TextCus body2 medium mainLightColor style={styles.dateSelectedText}>
        {data}
      </TextCus>
    );
    return (
